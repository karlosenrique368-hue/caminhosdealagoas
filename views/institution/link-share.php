...
<div class="mb-6">
    <p class="text-sm sm:text-base max-w-2xl" style="color:var(--text-secondary)">Compartilhe qualquer um destes links. Quando alguém comprar, a gente registra automaticamente que foi via você.</p>
</div>
...
<div class="admin-card p-5 sm:p-6 mb-6">
    ...
    <div class="space-y-3" x-data="{copied: null}">
        <?php foreach ([
            ['Site inteiro', $shareHome, 'Página inicial — funciona para tudo'],
            ['Todos os passeios', $sharePassis, 'Catálogo de passeios'],
            ['Todos os pacotes', $sharePacotes, 'Catálogo de pacotes'],
        ] as $idx => $l): ?>
        <div class="p-4 rounded-xl flex items-center gap-3 flex-wrap sm:flex-nowrap" style="background:var(--bg-surface);border:1px solid var(--border-default)">
            ...
            <div class="flex gap-2 w-full sm:w-auto">
                ...
                <a href="https://wa.me/?text=<?= urlencode('Vem viver Alagoas com a gente! ' . $l[1]) ?>" target="_blank" class="admin-btn admin-btn-secondary flex-1 sm:flex-initial justify-center">
...
<?php if ($roteiros): ?>
<div class="admin-card p-5 sm:p-6 mb-6">
    <h2 class="font-display text-lg font-bold mb-4 flex items-center gap-2" style="color:var(--sepia)"><i data-lucide="sparkles" class="w-5 h-5" style="color:var(--terracota)"></i> Links de passeios populares</h2>
    <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4" x-data="{c:null}">
        <?php foreach ($roteiros as $idx => $r):
            $url = referralShareUrl($code, '/passeios/'.$r['slug']);
        ?>
        <div class="rounded-xl overflow-hidden" style="background:var(--bg-surface);border:1px solid var(--border-default)">
            ...
            <div class="p-4">
                ...
                <div class="flex gap-1.5">
                    ...
                    <a href="https://wa.me/?text=<?= urlencode($r['title'] . ' - vem com a gente! ' . $url) ?>" target="_blank" class="admin-btn admin-btn-secondary justify-center text-xs" style="padding-left:10px;padding-right:10px"><i data-lucide="send" class="w-3.5 h-3.5"></i></a>
                </div>
            </div>
        </div>
        <?php endforeach; ?>
...
<div class="admin-card p-5 sm:p-6">
    ...
    <div class="grid sm:grid-cols-2 gap-3">
        <?php foreach ([
            ['message-circle','Grupos de WhatsApp','Cole o link nos grupos de família, trabalho, clube, igreja.'],
            ['instagram','Stories & bio','Adicione o link na bio do seu Instagram/TikTok.'],
            ['image','Prints dos passeios','Salve fotos da nossa página e compartilhe com seu link junto.'],
            ['heart','Indicação pessoal','Uma mensagem direta pra quem você sabe que vai amar.'],
        ] as $d): ?>
