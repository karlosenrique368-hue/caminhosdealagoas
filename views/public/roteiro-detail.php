...
// Avaliações REAIS (verificadas) deste passeio + fallback em testimonials destacados
$reviews = dbAll("
    SELECT r.id, r.rating, r.title, r.content, r.photos, r.created_at, r.verified, c.name
    FROM reviews r
    JOIN customers c ON c.id = r.customer_id
    WHERE r.entity_type='roteiro' AND r.entity_id=? AND r.status='approved'
    ORDER BY r.created_at DESC LIMIT 12", [$r['id']]);
$reviewsCount = (int)($r['rating_count'] ?? count($reviews));
$reviewsAvg   = $reviewsCount > 0 && !empty($r['rating_avg'])
    ? round((float)$r['rating_avg'], 1)
    : ($reviews ? round(array_sum(array_column($reviews,'rating')) / count($reviews), 1) : 0);
// Pode o cliente logado avaliar? (precisa ter booking pago)
$canReview = false; $reviewBookingId = null;
if (function_exists('isCustomerLoggedIn') && isCustomerLoggedIn()) {
    $cid = currentCustomerId();
    $bk = dbOne("SELECT b.id FROM bookings b LEFT JOIN reviews rv ON rv.booking_id=b.id AND rv.customer_id=? WHERE (b.customer_id=? OR b.customer_user_id=?) AND b.entity_type='roteiro' AND b.entity_id=? AND b.payment_status='paid' AND rv.id IS NULL ORDER BY b.id DESC LIMIT 1", [$cid, $cid, $cid, $r['id']]);
...
<section class="detail-content-section">
    <div class="max-w-7xl mx-auto px-4 sm:px-6">
        <div class="grid lg:grid-cols-3 gap-10">
            ...
            <div class="lg:col-span-2 space-y-10">
                ...
                <div class="admin-card p-6">
                    ...
                    <?php if (!empty($r['latitude']) && !empty($r['longitude'])): ?>
                        <div class="meeting-map" data-lat="<?= e($r['latitude']) ?>" data-lng="<?= e($r['longitude']) ?>" data-label="<?= e($r['meeting_point']) ?>" style="height:280px;border-radius:12px;overflow:hidden;border:1px solid var(--border-default)"></div>
                        <a href="https://www.google.com/maps/dir/?api=1&destination=<?= urlencode($r['latitude'].','.$r['longitude']) ?>" target="_blank" rel="noopener" class="inline-flex items-center gap-1.5 mt-3 text-sm font-semibold" style="color:var(--horizonte)"><i data-lucide="navigation" class="w-4 h-4"></i> Como chegar (Google Maps)</a>
                    <?php endif; ?>
                </div>
                <?php endif; ?>
                ...
                <div id="calendario" class="admin-card p-8" x-data="availabilityCalendar(<?= htmlspecialchars(json_encode([
                    ...
                    <div class="flex items-start justify-between flex-wrap gap-4 mb-6">
                        ...
                        <div class="flex items-center gap-2">
                            <button type="button" @click.stop="prevYear()" class="w-9 h-9 md:w-10 md:h-10 rounded-lg border flex items-center justify-center transition" style="border-color:var(--border-default);color:var(--text-secondary)" aria-label="Ano anterior"><i data-lucide="chevrons-left" class="w-4 h-4"></i></button>
                            <button type="button" @click="prevMonth()" class="w-9 h-9 md:w-10 md:h-10 rounded-lg border flex items-center justify-center transition" style="border-color:var(--border-default);color:var(--text-secondary)"><i data-lucide="chevron-left" class="w-4 h-4"></i></button>
                            ...
                            <button type="button" @click="nextMonth()" class="w-9 h-9 md:w-10 md:h-10 rounded-lg border flex items-center justify-center transition" style="border-color:var(--border-default);color:var(--text-secondary)"><i data-lucide="chevron-right" class="w-4 h-4"></i></button>
                            <button type="button" @click.stop="nextYear()" class="w-9 h-9 md:w-10 md:h-10 rounded-lg border flex items-center justify-center transition" style="border-color:var(--border-default);color:var(--text-secondary)" aria-label="Próximo ano"><i data-lucide="chevrons-right" class="w-4 h-4"></i></button>
                        </div>
                    </div>
            ...
            <aside class="lg:sticky lg:top-28 lg:self-start space-y-5">
                <div class="admin-card p-6">
                    ...
                    <button type="button" onclick="window.cart.addSelectedOrAsk('roteiro', <?= (int)$r['id'] ?>, '<?= e(addslashes($r['title'])) ?>')" class="mt-3 w-full inline-flex items-center justify-center gap-2 px-4 py-3 rounded-xl border-2 font-semibold text-sm transition hover:scale-[1.02]" style="color:var(--horizonte);border-color:var(--horizonte);background:rgba(58,107,138,0.05)">
        ...
        <div class="mt-20" x-data="reviewSection({entityType:'roteiro', entityId:<?= (int)$r['id'] ?>, bookingId:<?= (int)($reviewBookingId ?: 0) ?>})">
            <div class="flex items-end justify-between flex-wrap gap-4 mb-8">
                <div>
                    ...
                    <?php else: ?>
                    <p class="text-sm" style="color:var(--text-muted)">Seja o primeiro a avaliar este passeio.</p>
                    <?php endif; ?>
                </div>
                <?php if ($canReview): ?>
                    <button type="button" @click="formOpen=true" class="btn-primary"><i data-lucide="pen-square" class="w-4 h-4"></i> Avaliar este passeio</button>
                <?php endif; ?>
            </div>

            <!-- Formulário de avaliação (apenas se cliente logado e pago) -->
            <?php if ($canReview): ?>
