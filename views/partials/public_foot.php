...
(function(){
    ...
    function dict(text, lang){
        var clean = normalize(text);
        if (DICT[lang] && DICT[lang][clean]) return DICT[lang][clean];
        for (var sourceLang in DICT) {
            for (var sourceText in DICT[sourceLang]) {
                if (normalize(DICT[sourceLang][sourceText]) === clean) {
                    return (DICT[lang] && DICT[lang][sourceText]) || sourceText;
                }
            }
        }
        return null;
    ...
    function isTranslatable(node){
        ...
        if (!v) return false;
        var source = ORIGINAL_TEXT.get(node) || v;
        var s = source.trim();
    ...
    async function translateChunk(items, applyFn){
        ...
        try {
            var res = await fetch(ENDPOINT, {
                ...
                body: JSON.stringify({lang:current, texts:texts}),
    ...
    function updateLangUI(lang){
        var active = document.querySelector('[data-lang-option="' + (window.CSS && CSS.escape ? CSS.escape(lang) : lang) + '"]');
        document.querySelectorAll('[data-lang-option]').forEach(function(el){
            var isActive = el.dataset.langOption === lang;
            el.classList.toggle('is-active', isActive);
            el.setAttribute('aria-current', isActive ? 'true' : 'false');
            var check = el.querySelector('[data-lang-check]');
            if (check) check.style.display = isActive ? '' : 'none';
        });
        if (!active) return;
        document.querySelectorAll('[data-lang-current-flag]').forEach(function(img){ img.src = active.dataset.langFlag || img.src; });
        document.querySelectorAll('[data-lang-current-code]').forEach(function(el){ el.textContent = active.dataset.langSigla || lang; });
        document.querySelectorAll('[data-lang-current-name]').forEach(function(el){ el.textContent = dict('Idioma', lang) || 'Idioma'; });
    ...
    function start(){
        ...
        document.addEventListener('click', function(e){
            ...
            var alpineRoot = a.closest('[x-data]');
            if (window.Alpine && alpineRoot) { try { Alpine.$data(alpineRoot).open = false; } catch(err) {} }
        }, true);
    ...
    window.CaminhosI18n = { run: run, switchLang: switchLang, updateLangUI: updateLangUI };
