...
try {
    $row = passwordResetConsume('customer', $token, true);
    if (!$row) jsonResponse(['ok' => false, 'msg' => 'Link inválido ou expirado. Solicite um novo.']);

    $userId = (int)$row['user_id'];
    $hash = password_hash($pass, PASSWORD_DEFAULT);
    dbExec('UPDATE customers SET password_hash=?, updated_at=NOW() WHERE id=?', [$hash, $userId]);

    jsonResponse(['ok' => true, 'msg' => 'Senha redefinida com sucesso!', 'redirect' => url('/conta/login')]);
} catch (\Throwable $e) {
    error_log('[account-reset] ' . $e->getMessage());
